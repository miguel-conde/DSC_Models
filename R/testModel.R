...
sampleFiles(blogsTestingFile,   blogsSampleTestFile_1G,   p = p_blogs)
sampleFiles(newsTestingFile,    newsSampleTestFile_1G,    p = p_news)
sampleFiles(twitterTestingFile, twitterSampleTestFile_1G, p = p_twitter)
...
create_dir(testingCorpusTDMsDirectory)
...
(test_en_US_1Ga <- VCorpus(DirSource(samplesTestDirectory, encoding = "UTF-8"),
                  readerControl = list(language = "en")))

# Process punctuation
test_en_US_1Ga <- tm_map(test_en_US_1Ga, goodPunct)

# Save corpus
save(test_en_US_1Ga, file = testCorpusFile_1Ga)
...
test_tdm_1Ga <- TermDocumentMatrix(test_en_US_1Ga, control = ctrl1G)

save(test_tdm_1Ga, file = testTdmFile_1Ga)
...
corpus_en_US_test_4G <- tm_map(test_en_US_1Ga, addLimits)

tdm_test_4G <- TermDocumentMatrix(corpus_en_US_test_4G, control = ctrl)

save(tdm_test_4G, file = tdm_testFile_4G)
...
create_dir(testingModelsDirectory)
create_dir(testingBlogsModelDirectory)
create_dir(testingNewsModelDirectory)
create_dir(testingTwitterModelDirectory)
create_dir(testingTotalModelDirectory)

# BLOGS

tfl_Blogs_4G <- makeTFL(as.matrix(tdm_test_4G), N = 4, numDoc = 1, 
                        counts = TRUE, log = FALSE)

save(tfl_Blogs_4G, file = testingBlogsModelFile_4G)

# NEWS

tfl_News_4G <- makeTFL(as.matrix(tdm_test_4G), N = 4, numDoc = 2, 
                        counts = TRUE, log = FALSE)

save(tfl_News_4G, file = testingNewsModelFile_4G)

# TWITTER

tfl_Twitter_4G <- makeTFL(as.matrix(tdm_test_4G), N = 4, numDoc = 3, 
                       counts = TRUE, log = FALSE)

save(tfl_Twitter_4G, file = testingTwitterModelFile_4G)

## TOTAL
aux4 <- as.matrix(tdm_test_4G)
...
save(tfl_Total_4G, file = testingTotalModelFile_4G)
...
save(tfl_Blogs_4G,     file = mT.BlogsFile)
save(tfl_News_4G,      file = mT.NewsFile)
save(tfl_Twitter_4G,   file = mT.TwitterFile)
save(tfl_Total_4G,     file = mT.TotalFile)
